...
export default function ProjectDetailsPage() {
    ...
    const loadProjectData = async () => {
        try {
            ...
            if (!projectResponse.data.project_memberships || projectResponse.data.project_memberships.length === 0) {
                console.log('Using separate members response:', membersResponse.data);
                setProject(prev => prev ? { ...prev, project_memberships: membersResponse.data } : null);
            }
        } catch (error) {
    ...
    const isOwner = project.project_memberships.find(m => m.user_id === user?.id)?.role === 'owner';
                    ...
                    <div className="flex items-start justify-between">
                        ...
                        <div className="flex gap-3">
                            <button
                                ...
                                disabled={allUsers.filter(u => !project.project_memberships.find(m => m.user_id === u.id)).length === 0}
                                className="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                                title={allUsers.filter(u => !project.project_memberships.find(m => m.user_id === u.id)).length === 0 ? 'All users are already members' : 'Add a new member'}
                            >
                        ...
                        <div className="bg-white rounded-lg shadow">
                            ...
                            <div className="p-6">
                                <div className="space-y-4">
                                    {project.project_memberships.map((member) => (
                                        <div key={member.user_id} className="flex items-center justify-between">
                                            ...
                                            <div className="flex items-center gap-2">
                                                {editingMemberRole === member.user_id ? (
                                                    ...
                                                    <>
                                                        <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${member.role === 'owner' ? 'bg-purple-100 text-purple-800' : 'bg-gray-100 text-gray-800'
                                                            }`}>
                                                            {member.role}
                                                        </span>
                        ...
                        <div className="mt-3">
                            ...
                            <form onSubmit={handleAddMember} className="space-y-4">
                                <div>
                                    ...
                                    >
                                        <option value={0}>
                                            {allUsers.filter(u => !project.project_memberships.find(m => m.user_id === u.id)).length === 0
                                                ? 'All users are already members'
                                                : 'Select a user'
                                            }
                                        </option>
                                        {allUsers
                                            .filter(u => !project.project_memberships.find(m => m.user_id === u.id))
                                            .map((user) => (
                                                <option key={user.id} value={user.id}>
